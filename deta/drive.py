...
class Drive(_Service):
    def __init__(self, name:str=None, project_key:str=None, project_id:str=None, host:str=None):
    ...
    def _upload_part(self, name: str, upload_id: str, part: int, content_type:str=None):
        self._request(f"/uploads/{upload_id}?name={self._quote(name)}&part={part}", content_type=content_type)

    def _get_content_stream(self, data:typing.Union[str, bytes, TextIOBase, BufferedIOBase, RawIOBase]):
        if isinstance(data, str): return StringIO(data) 
        elif isinstance(data, bytes): return BytesIO(data)
        else: return data

    def put(self, name:str, data:typing.Union[str, bytes, TextIOBase, BufferedIOBase, RawIOBase]=None, *, path:str=None, content_type:str=None) -> str:
        """Put a file in drive.
        `name` is the name of the file.
